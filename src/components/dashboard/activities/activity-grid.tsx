...
import { categoryLabel } from '@/lib/activity-kinds'
...
export function ActivityCatalog({ summaries, tenantSlug }: ActivityCatalogProps) {
  ...
  const categories = React.useMemo(
    () => Array.from(new Set(summaries.map((s) => categoryLabel(s.activity)))).sort((a, b) => a.localeCompare(b)),
    [summaries],
  )
  ...
  const filtered = React.useMemo(() => {
    ...
    const rows = summaries.filter(({ activity }) => {
      ...
      if (category !== 'all' && categoryLabel(activity) !== category) return false

...
import {
  ...
  RevenueFeesChart,
  type PayoutBalance,
  type RevenueFeePoint,
} from '@/components/dashboard/payments/payout-summary'
import {
  PaymentsPageActions,
  PaymentsTable,
  type PaymentRow,
  type PayoutBatch,
} from '@/components/dashboard/payments/payments-table'
import {
  CURRENT_TENANT,
  NOW,
  PAYMENTS,
...
function bankLabel(tenantId: string, currency: CurrencyCode) {
...
function buildSettled(tenantId: string): SettledPayment[] {
...
function buildDailyPoints(settled: SettledPayment[]): RevenueFeePoint[] {
...
function buildPayouts(settled: SettledPayment[], destination: string): PayoutBatch[] {
...
function toPaymentRow(entry: SettledPayment): PaymentRow {
...
export default function PaymentsPage() {
  ...
  const paidThisMonth = payouts.filter(
    (batch) => batch.status === 'paid' && batch.paidAt.startsWith(monthPrefix),
  )
